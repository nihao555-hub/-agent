...
1. **E（本文）** 架构对齐。✅ 已完成
2. **A** 事实底座数据模型 + 仓储 + 图谱算子 + 项目级 API + 离线测试。✅ 已完成
3. **B** 带引用 RAG（embeddings 兜底 + 余弦检索 + 证据溯源）。✅ 已完成
4. **C** 文档理解（MinerU + IfcOpenShell 客户端 + 纯文本兜底）。✅ 已完成
5. **D** 变更/索赔自动组卷长流程（可断点续跑编排）。✅ 已完成
6. 贯穿：评测集 + tracing；逐层补测试，保持 lint/typecheck/build 全绿。（进行中：已补离线测试，评测集/tracing 待接 Langfuse）
